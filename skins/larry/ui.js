	...
	function init()
	{
		...
		if (rcmail.env.task == 'mail') {
			rcmail.addEventListener('menu-open', menu_toggle)
					...
					.addEventListener('responseafterlist', function (e) {
						switch_view_mode(rcmail.env.threading ? 'thread' : 'list', true)
					})
					.addEventListener('responseaftersearch', function (e) {
						switch_view_mode(rcmail.env.threading ? 'thread' : 'list', true)
					});
			...
			if (rcmail.env.action == 'show' || rcmail.env.action == 'preview') {
				rcmail.addEventListener('aftershow-headers', function () {
					layout_messageview();
				})
						.addEventListener('afterhide-headers', function () {
							layout_messageview();
						});
				...
				for (f = 0; f < fields.length; f++) {
					...
					field = $('#_' + v);
					if (field.length) {
						field.on('change', {v: v}, function (e) {
							if (this.value)
								show_header_row(e.data.v, true);
						});
						if (field.val() != '')
				...
				$("textarea[name='_to'], textarea[name='_cc'], textarea[name='_bcc']").change(function (e) {
					adjust_compose_editfields(this);
				}).change();
				rcmail.addEventListener('autocomplete_insert', function (p) {
					adjust_compose_editfields(p.field);
				});
				...
				$('#mailthreadmode').addClass(rcmail.env.threading ? 'selected' : '').click(function (e) {
					switch_view_mode('thread');
					return false;
				});
		...
		/***  settings task  ***/
			...
			rcmail.addEventListener('init', function () {
				...
				$(tab).addClass('selected')
						.children().first().removeAttr('onclick').click(function () {
					return false;
				});
			});
...
//    $('#composeformbuttons')[(btns ? 'show' : 'hide')]();
	...
	function show_listoptions(p)
	{
		...
		$dialog.dialog({
			...
			open: function (e) {
				setTimeout(function () {
					$dialog.find('a, input:not(:disabled)').not('[aria-disabled=true]').first().focus();
				}, 100);
			},
			close: function (e) {
	...
	function show_about(elem)
	{
		...
		if (supportln.length && (env.supporturl = supportln.attr('href')))
			buttons[supportln.html()] = function (e) {
				env.supporturl.indexOf('mailto:') < 0 ? window.open(env.supporturl) : location.href = env.supporturl
			};
...
function rcube_scroller(list, top, bottom)
{
	...
	this.top
			.mouseenter(function () {
				if (rcmail.drag_active)
					ref.ts = window.setTimeout(function () {
						ref.scroll('down');
					}, ref.delay);
			})
			.mouseout(function () {
				if (ref.ts)
					window.clearTimeout(ref.ts);
			});
	...
	this.scroll = function (dir)
	{
		...
		this.ts = window.setTimeout(function () {
			ref.scroll(dir);
		}, this.step_time);
	};
}
...
function rcube_splitter(p)
{
	...
	function onDrag(e)
	{
		...
		me.ts = window.setTimeout(function () {
			onDragAction(e);
		}, 1);
