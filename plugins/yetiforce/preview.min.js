...
function loadActionBar() {
  ...
  window.crm.AppConnector.request(params).done(function(response) {
    content.find(".ytHeader").html(response),
      $("#messagecontent").css(
        "top",
        content.outerHeight() + $("#messageheader").outerHeight() + "px"
      ),
...
function registerImportMail(content) {
  content.find(".importMail").click(function() {
    window.crm.Vtiger_Helper_Js.showPnotify({
...
function registerSelectRecord(content) {
  ...
  content.find("button.selectRecord").click(function() {
    var relParams = { mailId: id };
    if (0 == $(this).data("type")) {
      var module = $(this)
    ...
    showPopup(
      { module: module, src_module: "OSSMailView", src_record: id },
      relParams
    );
  });
}
function registerAddReletedRecord(content) {
  content.find("#mailActionBarID").val();
  content.find("button.addRelatedRecord").click(function(e) {
    var targetElement = $(e.currentTarget),
      row = targetElement.closest(".rowRelatedRecord"),
      params = { sourceModule: row.data("module") };
...
function registerAddRecord(content) {
  ...
  content.find("button.addRecord").click(function(e) {
    var col = $(e.currentTarget).closest(".js-head-container"),
      selectValue = col.find(".module").val();
    null !== selectValue && showQuickCreateForm(selectValue, id);
  });
}
function removeRecord(crmid) {
  ...
  (params.data = {
    ...
    mode: "removeRelated",
    params: { mailId: id, crmid: crmid }
  }),
    (params.async = !1),
    (params.dataType = "json"),
    window.crm.AppConnector.request(params).done(function(data) {
      var response = data.result,
        notifyParams = { text: response.data, animation: "show" };
      response.success &&
        (notifyParams = {
          text: response.data,
          type: "info",
...
function showPopup(params, actionsParams) {
  (actionsParams.newModule = params.module),
    window.crm.app.showRecordsList(params, function(modal, instance) {
      instance.setSelectEvent(function(responseData) {
        (actionsParams.newCrmId = responseData.id),
          window.crm.AppConnector.request({
            ...
            else var notifyParams = { text: response.data, animation: "show" };
            window.crm.Vtiger_Helper_Js.showPnotify(notifyParams),
              loadActionBar();
          });
      });
...
function showQuickCreateForm(moduleName, record) {
  var params =
      2 < arguments.length && arguments[2] !== void 0 ? arguments[2] : {},
    content = $("#ytActionBarContent"),
    relatedParams = {},
  ...
  var postShown = function(data) {
      $(
        ...
        $(
          '<input type="hidden" name="sourceRecord" value="' + record + '" />'
        ).appendTo(data),
        $(
  ...
  for (var i in ids) {
    var element = content.find("#" + ids[i]),
      value = element.length ? JSON.parse(element.val()) : [];
    0 <= $.inArray(sourceModule, value) && (relatedParams[i] = record);
  }
  "Leads" == moduleName && (relatedParams.company = rcmail.env.fromName),
    ("Leads" == moduleName || "Contacts" == moduleName) &&
      (relatedParams.lastname = rcmail.env.fromName),
    "Project" == moduleName && (relatedParams.projectname = rcmail.env.subject),
    "HelpDesk" == moduleName &&
...
function getCrmWindow() {
  if (null !== opener && "object" == opener.parent.CONFIG) return opener.parent;
  return "object" == _typeof(parent.CONFIG)
    ? parent
    : "object" == _typeof(parent.parent.CONFIG)
    ? parent.parent
    : !("object" != _typeof(opener.crm.CONFIG)) && opener.crm;
}
