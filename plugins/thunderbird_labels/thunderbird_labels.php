...
class thunderbird_labels extends rcube_plugin
{
	...
	function init()
	{
		...
		if ($this->rc->task == 'mail') {
			...
			if (method_exists($this, 'require_plugin') && in_array('contextmenu', $this->rc->config->get('plugins')) && $this->require_plugin('contextmenu') && $this->rc->config->get('tb_label_enable_contextmenu')) {
				if ($this->rc->action == '')
	...
	public function show_tb_label_contextmenu($args)
	{
		...
		$li = html::tag('li', array('class' => 'submenu'), '<span>' . rcube::Q($this->gettext('tb_label_contextmenu_title')) . '</span>' . $this->_gen_label_submenu($args, 'tb_label_ctxm_submenu'));
