...
class thunderbird_labels extends rcube_plugin
{
	...
	public function init()
	{
		...
		if ('mail' == $this->rc->task) {
			...
			$this->add_hook('messages_list', [$this, 'read_flags']);
			$this->add_hook('message_load', [$this, 'read_single_flags']);
			$this->add_hook('template_object_messageheaders', [$this, 'color_headers']);
			$this->add_hook('render_page', [$this, 'tb_label_popup']);
			$this->add_hook('check_recent', [$this, 'check_recent_flags']);
			...
			$this->register_action('plugin.thunderbird_labels.set_flags', [$this, 'set_flags']);
			...
			if (method_exists($this, 'require_plugin')
				...
				if ('' == $this->rc->action) {
					$this->add_hook('render_mailboxlist', [$this, 'show_tb_label_contextmenu']);
				}
			}
	...
	public function prefs_list($args)
	{
		...
		if (!\in_array('tb_label_custom_labels', $dont_override)
			...
			foreach ($custom_labels as $key => $value) {
				...
				$args['blocks']['tb_label']['options']["option_$key"] = [
					'title' => $key,
					'content' => $input->show()
				];
			}
	...
	private function _gen_label_submenu($args, $id)
	{
		...
		return html::tag('ul', ['id' => $id, 'role' => 'menu'], $out);
	}
	...
	public function template_include_markup($tpl_name)
	{
	...
	public function template_file2html($tpl_name)
	{
	...
	public function template2html($tpl_code)
	{
		if ('html' == $this->api->output->type) {
			if ($tpl_code) {
	...
	public function tb_label_popup($args)
	{
	...
	*/
	public function check_recent_flags($params)
	{
	 ...
	 *
	 * @param mixed $permanent_flags
	 */
	public function custom_flags_allowed($permanent_flags)
	{
		if (null !== $this->_custom_flags_allowed) { // primitive caching
	 ...
	 * @param mixed $permanent_flags
	 */
	public function custom_flags($permanent_flags)
	{
		$default_flags = [
	 ...
	 * @param mixed $flag
	 */
	public function roundcube_flag($flag)
	{
